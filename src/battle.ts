...
class Battle {
	...
	setAttrs() {
		...
		if (this.id.includes('multi') && this.p3 && this.p4) {
			...
			this.farSide.n = this.farSide.ally.n = 1;
		}
	}
	...
	runMajor(args: Args, kwArgs: KWArgs, preempt?: boolean) {
		...
		case 'turn': {
			...
			if (this.gameType === 'multi') {
				...
				const farSideSlot = this.farSide.sideid === 'p1' || this.farSide.sideid === 'p2' ? 0 : 1;
				let yourActives = [];
				yourActives[farSideSlot] = this.farSide.active[farSideSlot];
				yourActives[farSideSlot ^ 1] = this.farSide.ally.active[farSideSlot ^ 1];
				this.farSide.active = yourActives;
