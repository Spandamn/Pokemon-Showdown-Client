(function ($) {
	...
	var BattleRoom = this.BattleRoom = ConsoleRoom.extend({
		...
		updateControlsForPlayer: function () {
			...
			case 'team':
				if (this.battle.nearSide.pokemon && !this.battle.nearSide.pokemon.length) {
		...
		displayParty: function (switchables, trapped) {
			...
			if (this.battle.nearSide.ally) party += this.displayAllyParty();
			return party;
		},
		displayAllyParty: function () {
			var party = '';
			if (!this.battle.nearSide.ally) return;
			var allyParty = this.battle.nearSide.ally.myPokemon;
		...
		getPlayerChoicesHTML: function () {
			...
			if (this.choice.teamPreview) {
				...
				for (var i = 0; i < this.choice.choices.length; i++) {
					...
					case 'move':
						var move;
						nearActive = this.battle.nearSide.active;
		...
		updateSide: function (sideData) {
			...
			this.battle.farSide = this.battle.nearSide.foe;
			this.battle.nearSide.n = 0;
			this.battle.farSide.n = 1;
			if (this.battle.nearSide.ally) this.battle.nearSide.ally.n = 0;
			if (this.battle.farSide.ally) this.battle.farSide.ally.n = 1;
		},
		addAlly: function (allyData) {
			this.battle.nearSide.ally.myPokemon = allyData.pokemon;
			for (var i = 0; i < allyData.pokemon.length; i++) {
				...
				pokemonData.getHPColorClass = Pokemon.prototype.getHPColorClass;
				pokemonData.getHPColor = Pokemon.prototype.getHPColor;
				pokemonData.side = this.battle.nearSide.ally;
			}
		},
